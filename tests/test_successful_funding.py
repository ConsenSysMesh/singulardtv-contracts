...
class TestContract(TestCase):
    ...
    def test(self):
        ...
        # Backer 1 tries to steal revenue from backer 2 using withdrawRevenueFor but it fails.
        # Only SingularDTV token is allowed to withdraw revenue
        try:
            self.fund_contract.withdrawRevenueFor(accounts[BACKER_2], sender=keys[BACKER_1])
        except TransactionFailed:
            self.assertEqual(self.fund_contract.totalRevenue(), revenue)
        ...
        self.assertEqual(self.fund_contract.withdrawRevenue(reinvest, sender=keys[BACKER_1]), 0)
        ...
        self.assertEqual(self.fund_contract.withdrawRevenue(reinvest, sender=keys[BACKER_3]), 0)
